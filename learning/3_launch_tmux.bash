...
tmux send-keys -t "Fragile_Baseline:0" 'bash learning/launch.bash 1 2 "$@"; exit' C-m

# Create a new window and run script2.sh
tmux split-window -v
tmux send-keys -t 1 'conda activate mani && bash learning/launch.bash 3 4 "$@"; sleep 60; exit' C-m

# Create another window and run script3.sh
tmux split-window -v
tmux send-keys -t 2 'conda activate mani && bash learning/launch.bash 5 6 "$@"; sleep 60; exit' C-m
